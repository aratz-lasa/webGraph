...
def test_create_web_page():
    with open_graph() as graph:
        ...
        assert graph.exists_short_uri(web_page_url)
...
def test_delete_web_page_by_host():
    with open_graph() as graph:
        graph.create_short_uri_node(web_page_url)
        assert graph.exists_short_uri(web_page_url)
        graph.delete_short_uri(web_page_url)
        assert not graph.exists_short_uri(web_page_url)


def clean_web_pages(graph, hosts):
    for host in hosts:
        graph.delete_short_uri(host)
