...
from .flask_test_server import start_server_thread, host, port, path, ssl, html, status


headers_dictionary = {'Content-Type': ' text/html; charset=utf-8', 'Content-Length': ' 5', 'Server': ' Werkzeug/0.14.1 Python/3.7.2', 'Date': ' Sun, 10 Feb 2019 10:54:32 GMT'}
status_version = "HTTP/1.0 200 OK"
data = "Hello"
...
def prepare_raw_buffer_in(status_version=None, headers=None, data=None):
    raw_buffer_in = b""
    if status_version:
        raw_buffer_in = (status_version + HTTP_ONE_BLANK_LINE).encode("utf-8")
    if headers:
        headers_string = ""
        for key, value in headers.items():
            headers_string += key + ":" + value + HTTP_ONE_BLANK_LINE
        raw_buffer_in += headers_string.encode("utf-8")
    if data:
        if status_version or headers:
            raw_buffer_in += HTTP_ONE_BLANK_LINE.encode("utf-8")
        raw_buffer_in += data.encode("utf-8")
